...
public sealed class TimeGuardTests
{
    ...
    [Fact]
    public void AgainstNonUtc_ForDateTime_Should_Capture_ParamName_When_Not_Provided()
    {
        DateTime localInput = new(2026, 1, 1, 12, 30, 0, DateTimeKind.Local);

        Action action = () => TimeGuard.AgainstNonUtc(localInput);

        action.Should()
            .Throw<ArgumentException>()
            .Where(ex => ex.ParamName == nameof(localInput));
    }
    ...
    [Fact]
    public void AgainstNonUtc_ForDateTimeOffset_Should_Capture_ParamName_When_Not_Provided()
    {
        DateTimeOffset localInput = new(2026, 1, 1, 12, 30, 0, TimeSpan.FromHours(4));
